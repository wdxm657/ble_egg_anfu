...
**结果条目响应帧（最后一条，即 `entryIdx == totalEntriesInRecord - 1`），总长 16 字节**

最后一条 entry 恒为结果条目（`type=0x10` SUCCESS 或 `0x11` FAIL），其帧尾多 1 字节 `reward`，用于告知 APP 本次安抚成功是否附带零食投喂奖励。
...
| byte0～byte5   | 同单条帧  | 帧头                                              |
| byte6          | `status` | `0x00`                                            |
| byte7          | `entryIdx` | 最后一条索引（`totalEntriesInRecord - 1`）       |
| byte8          | `totalEntriesInRecord` | 同前                           |
| byte9          | `session_id` | 同前                                          |
| byte10         | `type`   | `0x10`=SUCCESS 或 `0x11`=FAIL                      |
| byte11～byte14 | `ts`     | 结果时间戳（Unix 秒，uint32 LE）                   |
| byte15         | `reward` | 零食奖励标记：`0x00`无；`0x01`=本次成功附带零食投喂奖励 |


> **reward 说明**：非结果条目（`entryIdx < totalEntriesInRecord - 1`）帧长为 9 字节、无该字段；仅结果条目为 10 字节。APP 可按 `entryIdx == totalEntriesInRecord - 1` 判断帧长并读取 `reward`。旧固件（无 reward 字段）的结果条目仍为 9 字节，读取时按 0 处理即可，向后兼容。
...
#### 4.20 零食奖励开关设置（REWARD_FLAG_SET，CMD = `0x35`）

**用途**：设置「安抚成功是否附带零食投喂奖励」功能开关。该开关由 MCU 保存在本地 Flash，每次启动时自动恢复；MCU 同步给 SOC 用于安抚成功时的奖励判定。
...
| byte0 | `0x01`   | 协议版本                                            |
| byte1 | `0x01`   | CMD                                                 |
| byte2 | `0x35`   | REWARD_FLAG_SET                                     |
| byte3 | `seq`    | 序号                                                |
| byte4 | `0x01`   | 负载长度 = 1                                        |
| byte5 | `0x00`   | 长度高字节                                          |
| byte6 | `enabled`| `0x00`=关闭，`非0`视为开启（固件规范化为 `0x01`）    |
...
| byte4 | `0x02`   | 负载长度 = 2                              |
| byte5 | `0x00`   | 长度高字节                                |
| byte6 | `status` | `0x00`成功；`0x03`参数错误                |
| byte7 | `enabledApplied` | 设备实际应用的开关值 `0x00`/`0x01` |


**参数错误时**：`byte6=0x03`（PARAM_ERROR），`byte7=0x00`。

---

#### 4.21 零食奖励开关查询（REWARD_FLAG_GET，CMD = `0x36`）

**用途**：查询当前零食奖励功能开关状态（进入设置页时恢复 UI 勾选态）。
...
| byte5 | `0x00`   | 长度高字节                                |
| byte6 | `status` | `0x00`成功；`0x03`参数错误                |
| byte7 | `enabled` | 当前开关值：`0x00`关闭，`0x01`开启       |
...
| byte17        | `reward`     | 零食奖励标记：`0x00`无，`0x01`=本次成功附带零食投喂奖励 |


> **reward 说明**：仅当 `result=0x01`（成功）且设备开启了「零食奖励」功能、且最后生效措施不是零食投喂时，设备会额外投喂一次零食并将 `reward=1`。该投喂**不属于安抚流程措施**（不产生 `0x82` 措施执行事件、不新增记录条目），APP 可用此字段展示「本次安抚附带零食奖励」。
